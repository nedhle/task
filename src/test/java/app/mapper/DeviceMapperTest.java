...
class DeviceMapperTest {
  ...
  void testDtoToEntity() {
    DeviceDto deviceDto = DeviceDto.builder().name("Test Device").brand("Test Brand").build();
  ...
  void testEntityToDtoRoundTrip() {
    ...
    Device device = deviceMapper.dtoToEntity(deviceDto);
    DeviceDto mappedBackDto =
        deviceMapper.requestToDto(new DeviceRequest(device.getName(), device.getBrand()));
